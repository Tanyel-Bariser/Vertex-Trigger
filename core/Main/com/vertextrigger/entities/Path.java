...
/**
 * Stores the predefined path of an entity's body & moves the body along it's predefined path at a distance based on the frame rate delta.
 */
public class Path {
	private Body body;

	/**
	 * Sets body & the body's predefined path. Sets entity's initial position & sets the first coordinate the entity should move to along it's
	 * predefined path.
	 *
	 * @param entity
	 * @param path
	 */
	public Path(final Body body, final Array<Vector2> coordinates) {
	...
	/**
	 * Moves body along it's predefined path by a distance dependent on delta for frame rate independent movement.
	 *
	 * @param delta
	 *            time passed between previous & current frame
	 */
	void moveAlongPath(final float delta) {
